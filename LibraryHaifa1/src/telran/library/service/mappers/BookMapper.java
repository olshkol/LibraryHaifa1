...
public class BookMapper extends AbstractMapper<BookEntity, Book> {
    ...
    void mapSpecificFieldsToEntity(Book book, BookEntity bookEntity) {
        bookEntity.setPublisher(publisherRepo.findById(book.getPublisherName()).orElse(null));
        Set<AuthorEntity> authors = new HashSet<>(authorRepo.findAllById(book.getAuthors()));
    ...
    void mapSpecificFieldsToDto(BookEntity bookEntity, Book book) {
        ...
        book.setAmountInUse(recordRepo.countByBookAndDateOfReturningNull(bookEntity));

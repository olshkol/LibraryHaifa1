...
public interface ILibrary extends ILogger {
    ...
    LibReturnCode pickupBook(long isbn, long readerId, LocalDate pickupDate);  //(OK, BOOK_NOT_EXISTS,
    ...
    List<Record> findRecordsByReader(long readerId, LocalDate from, LocalDate to);

    List<Record> findRecordsByReturnDate(LocalDate returnDate);

    List<Record> findOpenRecords();

    List<Book> getMostPopularBooks(LocalDate from_date, LocalDate to_date, int from_age, int to_age);

    List<PublisherAuthor> getMostPopularAuthors(LocalDate from, LocalDate to);
    ...
    Record getRecord(long isbn, long readerId, LocalDate datePickingingUp);
}
